...
enum class AccountStatus{mod,user,guest,troll};
...
bool permission_check(Action act, AccountStatus person)
{
        if(act==Action::read)
            return true;
        if(act==Action::write)
            return person != AccountStatus::guest;
    
        return person == AccountStatus::mod;
}
...
bool has_priority(AccountStatus a, AccountStatus b)
{
    ...
    if (a == AccountStatus::user)
        return true;

    if (b == AccountStatus::user)
        return false;

    if (a == AccountStatus::guest)
        return true;
